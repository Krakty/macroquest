...
void RegisterLocalPlayerBindings(sol::table& core)
{
	...
	localPlayer.set_function("GetCharacterBase", []() -> int {
		if (pLocalPlayer)
			return static_cast<int>(pLocalPlayer->SpawnID);
		return 0;
	});

	localPlayer.set_function("IsGateFlagSet", []() -> bool {
		// Gate flag - not directly accessible in eqlib
		// Return false as stub, will need investigation
		return false;
	});
}
