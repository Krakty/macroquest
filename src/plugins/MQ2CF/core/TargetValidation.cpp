...
void RegisterTargetBindings(sol::table& core)
{
	...
	target.set_function("IsValidHealTarget", [](int spawnId) -> bool {
		PlayerClient* pSpawn = FindSpawn(spawnId);
		if (!pSpawn)
			return false;
		// Must be in line of sight
		if (pLocalPlayer && !pLocalPlayer->CanSee(*static_cast<PlayerBase*>(pSpawn)))
			return false;
		// Must be in range (200 units default heal range)
		if (pLocalPlayer)
		{
			float dist = Distance3D(
				pLocalPlayer->Y, pLocalPlayer->X, pLocalPlayer->Z,
				pSpawn->Y, pSpawn->X, pSpawn->Z);
			if (dist > 200.0f)
				return false;
		}
		return true;
