...
void RegisterGroupBindings(sol::table& core)
{
	...
	group.set_function("GetGroupMembers", [](sol::this_state ts) -> sol::table {
		...
		for (int i = 0; i < MAX_GROUP_SIZE; ++i)
		{
			...
			if (pMember && pMember->pSpawn)
			{
				result[idx++] = static_cast<int>(pMember->pSpawn->SpawnID);
			}
		}
		return result;
	});

	group.set_function("GetMainAssist", []() -> int {
		CGroup* pGroup = GetGroup();
		if (!pGroup)
			return 0;
		CGroupMember* pMA = pGroup->GetGroupMemberByRole(GroupRoleAssist);
	...
	group.set_function("GetXTargets", [](sol::this_state ts) -> sol::table {
		sol::state_view lua(ts);
		sol::table result = lua.create_table();
		if (!pLocalPC || !pLocalPC->pExtendedTargetList)
			return result;
		...
		for (int i = 0; i < numSlots; ++i)
		{
			ExtendedTargetSlot* pSlot = pLocalPC->pExtendedTargetList->GetSlot(i);
			if (pSlot && pSlot->SpawnID != 0)
				result[idx++] = static_cast<int>(pSlot->SpawnID);
		}
		return result;
	});
