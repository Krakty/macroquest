...
void RegisterSpawnBindings(sol::table& core)
{
	...
	spawn.set_function("GetTarget", [](int spawnId) -> int {
		PlayerClient* pSpawn = FindSpawn(spawnId);
		if (!pSpawn)
			return 0;
		return static_cast<int>(pSpawn->TargetOfTarget);
	});
	...
	spawn.set_function("GetHPPercent", [](int spawnId) -> float {
		PlayerClient* pSpawn = FindSpawn(spawnId);
		if (!pSpawn)
			return 0.0f;
		int64_t maxHp = pSpawn->HPMax;
	...
	// -- HP Percent alias (matches Lua API name used in cleric.lua) --------
